...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Computing score between generated images and normal image')
    parser.add_argument('--normal_images_folder', type=str,default='./normal_output', help='Path to only schnell images folder')
    parser.add_argument('--generated_images_folder', type=str,default='./torchao_output', help='Path to generated images folder need to be compared')
    
    args = parser.parse_args()
    normal_path = args.normal_images_folder
    generated_path = args.generated_images_folder
    num_eval_images = len(os.listdir(normal_path))
    for i in range(num_eval_images):
        img1_path = os.path.join(normal_path, f'{i}.png')
        img2_path = os.path.join(generated_path, f'{i}.png')
        ...
        print(f"Clip embedding similarity prompt {i}: {clip_embedding_similiarity}")

        hash_distances = get_hash_distances(
            img1_response.image_hashes, img2_response.image_hashes
        )
        print(f"Hash distances prompt {i}: {hash_distances}")

        probability_same_image_xg = images_are_same_classifier.predict_proba(
                [hash_distances]
        )[0][1]

        print(f"Probability same image xg prompt {i}: {probability_same_image_xg}")
        ...
        print(f"Final score generated image {i}: {score**2}")
